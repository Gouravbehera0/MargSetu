    ...
    <div className="relative w-full overflow-hidden bg-slate-950 -mt-14 sm:-mt-16 transition-all duration-300 select-none z-0 border-b border-slate-800/80">
      ...
      <div
        className={`relative w-full transition-all duration-500 ease-in-out ${
          isCollapsed ? 'h-32 sm:h-36' : 'h-72 sm:h-84 lg:h-[26rem]'
        }`}
      >
        <img
          src="/images/highway_moving_vehicles.jpg"
          ...
          className={`w-full h-full object-cover object-center transform transition-transform duration-700 ${
            isCollapsed ? 'scale-105 opacity-45 brightness-75' : 'scale-100 opacity-95 brightness-95'
          }`}
        />

        {/* Seamless Gradients: Blends the top into the frosted navbar and bottom into the page */}
        <div className="absolute inset-0 bg-gradient-to-b from-slate-950/70 via-slate-950/20 to-slate-950/95 pointer-events-none" />
        <div className="absolute inset-0 bg-gradient-to-r from-slate-950/75 via-transparent to-slate-950/75 pointer-events-none" />
        ...
        <div className="absolute inset-0 max-w-7xl mx-auto px-4 sm:px-6 flex flex-col justify-between pt-16 sm:pt-20 pb-3 sm:pb-4">
          ...
          <div className="flex items-center justify-between z-10">
            <div className="flex items-center space-x-2.5">
              <div className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-blue-500/20 border border-cyan-400/40 text-cyan-300 backdrop-blur-md shadow-lg">
                <Radio className="w-4 h-4 text-cyan-300 animate-pulse" />
              </div>
              <div>
                <div className="flex items-center space-x-2">
                  <span className="text-xs sm:text-sm font-black text-white tracking-wide uppercase flex items-center gap-1.5 drop-shadow-md">
                  ...
                  <span className="hidden md:inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 backdrop-blur-md">
                    V2X Connected Highway
                  </span>
                </div>
                {!isCollapsed && (
                  <p className="text-[11px] text-slate-200/90 hidden sm:block mt-0.5 drop-shadow">
            ...
            <button
              ...
              className="px-2.5 py-1 sm:py-1.5 rounded-xl bg-slate-950/70 hover:bg-slate-900 text-slate-200 hover:text-white border border-slate-700/60 text-xs font-bold backdrop-blur-md transition-all flex items-center space-x-1.5 shadow-md group"
